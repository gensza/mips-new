...
$(document).ready(function(){
    ...
    var noid     = '<?php echo $id_rows;?>';
    var ref      = '<?php echo $id_rows2;?>';


    $.ajax({ 
            url: base_url + 'gl/transaksi_header_detail',
            type: "post",
    ...
    total_credit = function(){
        ...
        $.ajax({ 
            url: base_url + 'gl/transaksi_total_credit_edit',
            type: "post",
            data:{ref : ref,
              <?php echo $this->security->get_csrf_token_name(); ?>:'<?php echo $this->security->get_csrf_hash(); ?>'},
            dataType: "json",
  ...
  total_debit = function(){
        ...
        $.ajax({ 
            url: base_url + 'gl/transaksi_total_debit_edit',
            type: "post",
            data:{ref : ref,
              <?php echo $this->security->get_csrf_token_name(); ?>:'<?php echo $this->security->get_csrf_hash(); ?>'},
            dataType: "json",
                    ...
                    $("#no_ref").focus(); 
                      ...
                      Command: toastr["error"]("Silahkan pilih Kurs !", "Info, Ada kolom yang masih kosong !");
                        $("#dc_kurs").focus(); 
                            ...
                            $.ajax({
                                url             : base_url + 'gl/transaksi_update_dollar2', 
                                type            : "POST",
                                dataType        : 'json',
                                mimeType        : 'multipart/form-data',
                                data            : form_data,
          ...
          $("#no_ref").focus(); 
            ...
            Command: toastr["error"]("Silahkan pilih Kurs !", "Info, Ada kolom yang masih kosong !");
              $("#dc_kurs").focus(); 
                  ...
                  $.ajax({
                      url             : base_url + 'gl/transaksi_update2', 
                      type            : "POST",
                      dataType        : 'json',
                      mimeType        : 'multipart/form-data',
                      data            : form_data,
  ...
  $('.maskmoney_money_kurs').maskMoney({thousands:',', decimal:'.', precision:0,});


  tabel_gl_transaksi_detail = function(){
        
        $.ajax({ 
            url: base_url + 'gl/transaksi_data_detail_edit',
            type: "post",
            ...
            success: function(result)
            {
                ...
                for ( var i=0 ; i<result.length ; i++ ) {
                    ...
                    var link = '<a href="javascript:void(0)" onclick=edit_gl_trans_tmp('+result[i].noid+')><i class="splashy-document_a4_edit" style="cursor:pointer"></i></a> <a href="javascript:void(0)" onclick=hapus_gl_trans_tmp('+result[i].noid+')><i class="splashy-document_a4_remove" style="cursor:pointer"></i></a>';
...
edit_gl_trans_tmp = function(noid){
  ...
  $("#noid_trans_gl_details").val(noid);
      ...
      $.ajax({ 
            url: base_url + 'gl/get_trans_gl_detail2',
            type: "post",
            data:{noid : noid,
              <?php echo $this->security->get_csrf_token_name(); ?>:'<?php echo $this->security->get_csrf_hash(); ?>'},
            dataType: "json",
 ...
 hapus_gl_trans_tmp = function(noid){
  ...
  function(){
          ...
          $.ajax({ 
            url: base_url + 'gl/hapus_trans_gl_detail2',
            type: "post",
            data:{noid : noid,
              <?php echo $this->security->get_csrf_token_name(); ?>:'<?php echo $this->security->get_csrf_hash(); ?>'},
            dataType: "json",
  ...
  $("#btn_update_kurs").click(function(){
        ...
        $.ajax({ 
            ...
            type: "post",
            data:{tgl_kurs_todays:tgl_kurs_todays,kurs_nominal : $("#kurs_nominal").val(),
              <?php echo $this->security->get_csrf_token_name(); ?>:'<?php echo $this->security->get_csrf_hash(); ?>'},
            dataType: "json",
            async : 'false',
            success: function(result)
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                $kategori.append('<option value=' + data[i].afd + '>'+ data[i].nama +' </option>');
            }
            $kategori.append('<option value="-">-</option>');
        },
        beforeSend: function () {
  ...
  get_tahun = function(val_afd){
      ...
      $('#tahun_tanam').empty();
      var sss;
      if(kategori == 'PEMBIBITAN'){
        sss = val_afd.substr(0,4);
      }else{
        sss = val_afd;
      }
      ...
      $.ajax({
          ...
          data: {kategori:$("#kategori").val(),afd_unit:sss},
          dataType  : 'json',
          success: function (data) {
              var $kategori = $('#tahun_tanam');
              $kategori.append('<option value="0">- Pilih -</option>');
              for (var i = 0; i < data.length; i++) {
                  $kategori.append('<option value=' + data[i].val_tahun + '>'+ data[i].nama +' </option>');
              }
              $kategori.append('<option value="-">-</option>');
          },
          beforeSend: function () {
  ...
  document.getElementById("acctno").addEventListener("keyup", function(event) {
          ...
          if (event.keyCode === 13) {
            ...
            if($("#divisi_v").val() == '06'){
              ...
              var kategoris   = $("#kategori").val();
              var afdunits    = $("#afd_unit").val();
              var tahuntanams = $("#tahun_tanam").val();
              ...
              }else if(kategoris == 'TBM'){
                src_noac = afdunits+''+tahuntanams;
                getpopup('gl/master_tabel_coa_popup_by_kategori','<?php echo $this->session->userdata('sess_token');?>','popupedit','TBM',src_noac);
              }else if(kategoris == 'LAND_CLEARING'){
                src_noac = tahuntanams;
                getpopup('gl/master_tabel_coa_popup_by_kategori','<?php echo $this->session->userdata('sess_token');?>','popupedit','LAND_CLEARING',src_noac);
              }else if(kategoris == 'PEMBIBITAN'){
                src_noac = afdunits;
                getpopup('gl/master_tabel_coa_popup_by_kategori','<?php echo $this->session->userdata('sess_token');?>','popupedit','PEMBIBITAN',src_noac);
              }else{
                getpopup('gl/master_tabel_coa_popup','<?php echo $this->session->userdata('sess_token');?>','popupedit','1');
              }
              
              //var src_noac = afdunits+''+tahuntanams;
              //alert(src_noac)
...
$("#btn_back").click(function(){
        ...
        getcontents('gl/transaksi_table','<?php echo $this->session->userdata('sess_token');?>');
});
...
<form id="form_input_transaksi"  method=POST enctype='multipart/form-data' style="background:#fff59d;padding:15px;border-radius:10px">
...
<h3 class="heading"><span id="btn_back" class="btn btn-default" style="color:red;cursor:pointer" ><i class="splashy-arrow_large_left"></i> Kembali </span> <span style="font-weight:bold">| GL : Edit Transaksi</span></h3>
  ...
  <div class="span12" >
      ...
      <div class="span4" >
            ...
            <div class="span4">
                ...
                <label for="demo-vs-definput" class="control-label">Ref #</label>
                    <input type="text" class="form-control teksbesar span17" name="no_ref" id="no_ref" readonly>
                </div>
            </div>
            ...
            <div class="span4">
                ...
                <label for="demo-vs-definput" class="control-label">Tanggal</label>
                  ...
                  <input type="text" size="20" class="span9" id="tanggal" name="tanggal" readonly><span class="add-on" id="datepicker_pointer" style="cursor: pointer;"><i class="icon-calendar"></i></span>
                 </div> 
                </div>
            </div>
        </div>
      ...
      <div class="span12" >
            ...
            <div class="span1">
                ...
                <select class="form-control span17 reset" name="dc_kurs" id="dc_kurs">
                                            <option value="0">-Pilih-</option>
      ...
      <table id="tabel_detail_transaksi_gl" class="table table-hover table-striped table-bordered" style="width: 100%">
                                      <thead>
                                          <tr>
                                              ...
                                              <th style="width: 10%">Nama Acc</th>
                                              <th style="width: 10%">Ket. Transaksi</th>
