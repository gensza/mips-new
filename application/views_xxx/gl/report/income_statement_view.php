...
$(document).ready(function(){
    ...
    $("#btn_tampilkan_popup").click(function(){
      ...
      $("#btn_cetak").show();
        ...
        if(kategori == 'summary'){ // by group
            var url  = '<?php echo base_url('gl_lap/income_summary/');?>';
            newwindow=window.open(url+'/'+periode+'/'+bygroup+'/'+kategori,'Laporan Trial Balance By Group','_blank');
            if (window.focus) {newwindow.focus()}
            return false;
        }else if(kategori == 'detail_biaya_langsung_kebun'){ // by trial detail
            var url  = '<?php echo base_url('gl_lap/income_detail_biaya_langsung_kebun/');?>';
            newwindow=window.open(url+'/'+periode+'/'+bygroup+'/'+kategori,'Laporan Trial Balance','_blank');
            if (window.focus) {newwindow.focus()}
            return false;
        }else if(kategori == 'laporan_tahun_semua_level'){ // by trial detail
            var url  = '<?php echo base_url('gl_lap/income_tahun_semua_level/');?>';
            newwindow=window.open(url+'/'+periode+'/'+bygroup+'/'+kategori,'Laporan Trial Balance','_blank');
            if (window.focus) {newwindow.focus()}
            return false;
        }else if(kategori == 'level_2'){ // by level 2
            var url  = '<?php echo base_url('gl_lap/income_tahun_semua_by_level/');?>';
            var cbx_level = '2';
            newwindow=window.open(url+'/'+periode+'/'+cbx_level+'/'+kategori,'Laporan Trial Balance','_blank');
            if (window.focus) {newwindow.focus()}
            return false;
        }else if(kategori == 'level_3'){ // by level 2
            var url  = '<?php echo base_url('gl_lap/income_tahun_semua_by_level/');?>';
            var cbx_level = '3';
            newwindow=window.open(url+'/'+periode+'/'+cbx_level+'/'+kategori,'Laporan Trial Balance','_blank');
            if (window.focus) {newwindow.focus()}
            return false;
        }else if(kategori == 'level_4'){ // by level 2
            var url  = '<?php echo base_url('gl_lap/income_tahun_semua_by_level/');?>';
            var cbx_level = '4';
            newwindow=window.open(url+'/'+periode+'/'+cbx_level+'/'+kategori,'Laporan Trial Balance','_blank');
            if (window.focus) {newwindow.focus()}
            return false;
        }else if(kategori == 'level_5'){ // by level 2
            var url  = '<?php echo base_url('gl_lap/income_tahun_semua_by_level/');?>';
            var cbx_level = '5';
            newwindow=window.open(url+'/'+periode+'/'+cbx_level+'/'+kategori,'Laporan Trial Balance','_blank');
            if (window.focus) {newwindow.focus()}
...
<input type='hidden' class='form-control' name='<?php echo $this->security->get_csrf_token_name(); ?>' value="<?php echo $this->security->get_csrf_hash(); ?>">
  ...
  <div class="row-fluid">
  <div class="span6" >
      <h3 class="heading">Laporan Laba Rugi</h3>
          ...
          <div class="row-fluid">
            <div class="span12">
                <input type="radio" name="kategori" value="summary" checked="checked"> Summary<br>
                <input type="radio" name="kategori" value="detail_biaya_langsung_kebun"> Detail Biaya Langsung Kebun<br>
<!--                <input type="radio" name="kategori" value="grafik_perbulan" id="TBL"> Grafik Perbulan<br>
                <input type="radio" name="kategori" value="grafik_pertahun" id="TBP"> Grafik Pertahun<br>-->
                <input type="radio" name="kategori" value="laporan_tahun_semua_level" id="TBP"> Laporan Tahun Semua Level<br>
                <input type="radio" name="kategori" value="level_2" id="lvl_2"> Level 2<br>
                <input type="radio" name="kategori" value="level_3" id="lvl_3"> Level 3<br>
                <input type="radio" name="kategori" value="level_4" id="lvl_4"> Level 4<br>
                <input type="radio" name="kategori" value="level_5" id="lvl_5"> Level 5<br>
            </div>
          </div>
