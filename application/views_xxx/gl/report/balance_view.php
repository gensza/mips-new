...
//        Balance Tahun Level 3
        ...
        if(kategori == 'rb_1'){ // current year
            ...
            newwindow=window.open(url+'/'+periode+'/0/'+kategori,'Laporan Balance Sheet','_blank');
            if (window.focus) {newwindow.focus()}
            return false;
        }else if(kategori == 'rb_2'){ // compare year
            var url  = '<?php echo base_url('gl_lap/balance_compare_year/');?>';
            newwindow=window.open(url+'/'+periode+'/0/'+kategori,'Laporan Balance Sheet Compare Year','_blank');
            if (window.focus) {newwindow.focus()}
            return false;
        }else if(kategori == 'rb_3'){ // by trial detail
            var url  = '<?php echo base_url('gl_lap/balance_tahun_level_2/');?>';
            newwindow=window.open(url+'/'+periode+'/2/'+kategori,'Laporan Balance Sheet','_blank');
            if (window.focus) {newwindow.focus()}
            return false;
        }else if(kategori == 'rb_4'){ // by level 2
            var url  = '<?php echo base_url('gl_lap/balance_tahun_level_2/');?>';
            newwindow=window.open(url+'/'+periode+'/3/'+kategori,'Laporan Balance Sheet','_blank');
            if (window.focus) {newwindow.focus()}
            return false;
        }
