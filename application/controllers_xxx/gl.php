...
class Gl extends CI_Controller {
    ...
    public function get_nama_acct(){
        ...
        $result = $this->gl_model->get_nama_acct($data)->row_array();
        echo json_encode($result);
    }
    ...
    function get_afd_unit(){
        ...
        $result = $this->gl_model->get_afd_unit($kategori)->result_array();
        echo json_encode($result);
    ...
    function get_tahuntanam(){
        ...
        $result = $this->gl_model->get_tahuntanam($kategori,$afd_unit)->result_array();
        echo json_encode($result);
    }
        ...
        foreach ($res_data_acct as $v) {
            ...
            $crdt = $a['CREDIT_F2'];
                    ...
                    if($crdt > 0){
                        ...
                        $hasilcr = preg_replace('/\-([0-9,.]+)/', '(\1)', number_format($a['CREDIT_F2'], 0));
                       // $hasilcr = 'if';
                    }else{
                        //$hasilcr = 'else';
                        $hasilcr =  number_format($a['CREDIT_F2'], 0, ".", ",");
            ...
            $html .= '<tr style="border-bottom:1pt solid black;">
                ...
                <td align="right" width="10%"><div style="float:right;border-right:1pt solid black;">'.number_format($a['DEBET_F2'], 0, ".", ",").'</div></td>
...
//        
//        
//        if {entry.group}="Asset" or {entry.group}="Expenses" OR {entry.group}="Other Expenses" then 
//            (if {@ttldr}+{@ttlcr}>0 THEN {@ttldr}+{@ttlcr}
//            ELSE
//            0)
//        ELSE
//           (if {@ttlcr}+{@ttldr}>0 THEN 0 
//            ELSE
//           {@ttlcr}+{@ttldr})
//        
        ...
        if($ttlcr == '0'){
            ...
            $ttlcr_v = preg_replace('/\-([0-9,.]+)/', '(\1)', number_format($ttlcr, 0));
        ...
        if($curbalcr == '0'){
            ...
            $curbalcr_v = preg_replace('/\-([0-9,.]+)/', '(\1)', number_format($curbalcr, 0));
...
//        }
        ...
        <td align="right" width="150px" style="background: ;color: black;font-weight: bold;border-right:1pt solid black;"><div style="float:right">'. number_format($ttldr, 0, ".", ",").'</div></td>
        <td align="right" width="150px" style="background:;color: black;font-weight: bold"><div style="float:right">'.$ttlcr_v.'</div></td>
        </tr>
        <tr>  
        <td width="100px" colspan="3" style="text-align: right;color:black;font-weight: bold;">Current Balance</td>
        <td align="right" width="150px" style="background:;color: black;font-weight: bold;border-right:1pt solid black;"><div style="float:right">'.number_format($curbal, 0, ".", ",").'</div></td>
