...
class Gl_model extends CI_Model
{
    ...
    function get_data_entry_head_rpt_module_lpbbkb($tgl_start, $tgl_end, $periode_terkini, $divisi_start, $divisi_end, $module)
    {
        ...
        if ($periode_terkini == 1) { //ini artinya pakai periode, filter tanggal tidak berlaku
            ...
            $sql = "SELECT sbu, noac, descac, ref,SUM(dr) AS DBT_NF,FORMAT(SUM(dr), 2) DBT,FORMAT(SUM(cr), 2) KRD,SUM(cr) KRD_NF FROM entry WHERE noac LIKE '%1025%' AND periodetxt = '$sess_periode' $dv_start $modules GROUP BY noac, sbu ORDER BY `DATE`, ref ASC ";
            return $this->mips_gl->query($sql);
        } else {

            $sql = "SELECT sbu, noac, descac, ref,SUM(dr) AS DBT_NF,FORMAT(SUM(dr), 2) DBT,FORMAT(SUM(cr), 2) KRD,SUM(cr) KRD_NF FROM entry WHERE noac LIKE '%1025%' AND DATE(`DATE`) >= STR_TO_DATE('$tgl_start', '%d-%m-%Y') AND DATE(`DATE`) <= STR_TO_DATE('$tgl_end', '%d-%m-%Y') $dv_start $modules GROUP BY ref ORDER BY `DATE`, ref ASC ";
            return $this->mips_gl->query($sql);
        }
    }
    ...
    function posting_harian_general()
    {
        ...
        foreach ($res_sql_general_master as $a) {
            ...
            if ($a['group'] == 'Asset' || $a['group'] == 'Expenses' || $a['group'] == 'Other Expenses') {
                ...
                // jika TTLCR tidak sama dengan 0 dan TTLDB tidak sama dengan 0
                if ($y['TTLCR'] <> 0 && $y['TTLDB'] <> 0) {
                    $TOTALDR = $y['TTLDB'] - $y['TTLCR'];
                    if ($TOTALDR > 0) {
                        $TOTALDR = $TOTALDR;
    ...
    function trx_to_posting_harian($data)
    {
        //ini group dulu berdasarkan noac
        $period = $this->periode();
        $periodes = substr($this->session->userdata('sess_periode'), 0, 4) . '-' . substr($this->session->userdata('sess_periode'), 4, 6);
        ...
        foreach ($sql_entry as $entry) {
            ...
            if ($GROUPS == 'Asset' || $GROUPS == 'Expenses' || $GROUPS == 'Other Expenses') {
                ...
                //sum cr di entry dan sum dr di entry
                if ($entry['SumOfcr'] <> 0 && $entry['SumOfdr'] <> 0) {
            ...
            //saldo yang ada ditambahkan
            $sql_noac = "SELECT $DEBIT, $KREDIT
            FROM noac
            WHERE noac = '$KODECOA'";
            $gege = $this->mips_gl->query($sql_noac)->row();
            ...
            $sql3 = "UPDATE noac SET balancedr  = 0,
                                         balancecr  = 0,
                                         $DEBIT = '$data_d' + '$TOTALDR1',
                                         $KREDIT= '$data_c' + '$TOTALCR1' 
                                        WHERE noac  = '$KODECOA'";
            $this->mips_gl->query($sql3);
    ...
    function trx_to_posting_harian_general($data)
    {
        ...
        foreach ($sql_entry as $data) {
            ...
            foreach ($res_sql_general_master as $a) {
                ...
                if ($a['group'] == 'Asset' || $a['group'] == 'Expenses' || $a['group'] == 'Other Expenses') {
                    ...
                    // sum cr di noac dan sum dr di noac
                    if ($y['TTLCR'] <> 0 && $y['TTLDB'] <> 0) {
                        $TOTALDR = $y['TTLDB'] - $y['TTLCR'];
                        if ($TOTALDR > 0) {
                            $TOTALDR = $TOTALDR;
    ...
    function posting_transfer_trx_to_ho()
    {
        ...
        $del = "DELETE FROM db_mips_gl_" . $db_pt . ".entry WHERE periode = '$periode_nya' $lok";
        $this->mips_gl->query($del);

        //delete header entry yang saudah ada di gl ho
        $del_header = "DELETE FROM db_mips_gl_" . $db_pt . ".header_entry WHERE periode = '$periode_nya' $lok";
        $this->mips_gl->query($del_header);

        //insert entry copy dari gl unit ke GL HO
        $query = "INSERT INTO db_mips_gl_" . $db_pt . ".entry SELECT * FROM db_mips_gl_msal_" . $unit_nya . ".entry WHERE periode = '$periode_nya'";
        $this->mips_gl->query($query);

        //insert header entry copy dari gl unit ke GL HO
        $query_header = "INSERT INTO db_mips_gl_" . $db_pt . ".header_entry SELECT * FROM db_mips_gl_msal_" . $unit_nya . ".header_entry WHERE periode = '$periode_nya'";
