...
class Gl_model extends CI_Model{	
    ...
    if($data['dc'] == 'C'){
        ...
        $sql = "INSERT INTO entry_temp (ref,noref,
                            ...
                            VALUES ('$data[no_ref]',
                                    ...
                                    '$dt[type]',
                                    '$dt[level]',
                                    '$dt[general]','$sess_lokasi','$data[dc_kurs]')";
    ...
    function transaksi_simpan_all($data){
        ...
        $sql2 = "SELECT * FROM entry_temp where noref = '$data[kode_sementara]'";
        $n = $this->mips_gl->query($sql2)->result_array();
        // fungsi insert batch
        ...
        $sql = "INSERT INTO header_entry (`date`,
                                    periode,
                                    ref,
                                    totaldr,
                                    totalcr,
                                    lokasi,
                                    periodetxt) 
                            VALUES (STR_TO_DATE('$data[tanggal]', '%d-%m-%Y'),
                                    STR_TO_DATE('$data[tanggal]', '%d-%m-%Y'),
                                    '$data[no_ref]',
                                    '$data[totaldr]',
                                    '$data[totalcr]',
    ...
    function master_update($data){
        ...
        function get_afd_unit($kategori){
            ...
            /* KODE PADA ACCOUNT COA :

            22 = TBM (TANAMAN BELUM MENGHASILKAN)
            23 = TM  (TANAMAN MENGHASILKAN)
            26 = LC  (LAND CLEARING)
            27 = PEMBIBITAN / PABRIK
            
            */

            if($kategori == 'TBM'){ //TBM

                $sql = "SELECT noac,nama,SUBSTR(noac,1,4) AS afd,SUBSTR(noac,3,2) AS val_afd FROM noac WHERE SUBSTR(noac,1,2)  = '22' AND SUBSTR(noac,5,2) = '00' AND SUBSTR(noac,3,8) <> '00000000'";
                //return $this->mstcode->query($sql);
                return $this->mips_gl->query($sql);

            }else if($kategori == 'TM'){ //TM

                $sql = "SELECT noac,nama,SUBSTR(noac,1,4) AS afd,SUBSTR(noac,3,2) AS val_afd FROM noac WHERE SUBSTR(noac,1,2)  = '23' AND SUBSTR(noac,5,2) = '00' AND SUBSTR(noac,3,8) <> '00000000'";
                //return $this->mstcode->query($sql);
                return $this->mips_gl->query($sql);
                
            }else if($kategori == 'PEMBIBITAN'){ // PEMBIBITAN

                $sql = "SELECT noac,nama,SUBSTR(noac,1,4) AS afd_asli,CONCAT(SUBSTR(noac,1,4),SUBSTR(noac,5,2)) afd,SUBSTR(noac,5,2) AS val_tahun FROM noac WHERE SUBSTR(noac,1,4) = '2714' AND SUBSTR(noac,7,9) = '0000' AND SUBSTR(noac,5,2) <> '00'";
                //return $this->mstcode->query($sql);
                return $this->mips_gl->query($sql);

            }else if($kategori == 'LAND_CLEARING'){ // LAND CLEARING

                //ini sengaja dibuat error, karna Land Clearing itu ga pake Afd/Unit, Value Filternya dibuat : 0000XXXXXXX
                $sql = "SELECT noac,nama,SUBSTR(noac,1,4) AS afd,SUBSTR(noac,5,2) AS val_tahun FROM noac WHERE SUBSTR(noac,1,4) = '0000XXXXXXX' AND SUBSTR(noac,7,9) = '0000XXXXXXX' AND SUBSTR(noac,5,2) <> '0000XXXXXXX'";
                //return $this->mstcode->query($sql);
                return $this->mips_gl->query($sql);
            ...
            //,CONCAT(SUBSTR(noac,1,4),SUBSTR(noac,5,2)) AS afd
        ...
        function get_tahuntanam($kategori,$afd_unit){
            ...
            */
            ...
            if($kategori == 'TBM'){//TBM
                ...
                $sql = "SELECT noac,nama,SUBSTR(noac,5,2) AS val_tahun FROM noac WHERE SUBSTR(noac,1,4) = '$afd_unit' AND SUBSTR(noac,5,2) <> '00' AND SUBSTR(noac,7,9) = '0000'";
                //return $this->mstcode->query($sql);
                return $this->mips_gl->query($sql);
                ...
                $sql = "SELECT noac,nama,SUBSTR(noac,1,4) AS val_tahun FROM noac WHERE SUBSTR(noac,1,4) = '$afd_unit' AND SUBSTR(noac,5,2) = '00'";

                //$sql = "SELECT noac,nama,SUBSTR(noac,5,2) AS val_tahun FROM noac WHERE SUBSTR(noac,1,4) = '$afd_unit' AND SUBSTR(noac,7,9) = '0000' AND SUBSTR(noac,5,2) = '00'";
                //return $this->mstcode->query($sql);
                return $this->mips_gl->query($sql);

            }else if($kategori == 'LAND_CLEARING'){ //LAND CLEARING

                //$sql = "SELECT noac,nama,SUBSTR(noac,5,9) AS val_tahun FROM noac WHERE SUBSTR(noac,1,2) = '26' AND SUBSTR(noac,5,9) = '000000'";
                //CONCAT(SUBSTR(noac,1,4),LPAD(@rownr:=@rownr+1, 2, 0)) val_tahun
                $sql = "SELECT nama,SUBSTR(noac,1,4) AS val_tahun,SUBSTR(noac,3,2) AS val_tahun_asli4 FROM noac WHERE SUBSTR(noac,1,2) = '26' AND SUBSTR(noac,5,9) = '000000' AND SUBSTR(noac,1,4) <> '2600'";
                //return $this->mstcode->query($sql);
                return $this->mips_gl->query($sql);
            }
        ...
        function get_data_entry($periode_terkini,$tgl_start,$tgl_end,$divisi_start,$divisi_end,$noacc_start,$noacc_end){
            ...
            if($periode_terkini == 1){ //ini artinya pakai periode, filter tanggal tidak berlaku
                ...
                $sql = "SELECT *,DATE_FORMAT(`DATE`, '%d-%m-%Y') TGL,cr CREDIT_F2 ,dr DEBET_F2,FORMAT(dr, 2) DEBET_F,FORMAT(cr, 2) CREDIT_F FROM entry WHERE SUBSTR(periodetxt,1,6) = '$sess_periode' AND lokasi = '$sess_lokasi' $dv_start $acc ORDER BY `DATE`,dr DESC";
                return $this->mips_gl->query($sql);

            }else{

                $sql = "SELECT *,DATE_FORMAT(`DATE`, '%d-%m-%Y') TGL,cr CREDIT_F2,dr DEBET_F2,FORMAT(dr, 2) DEBET_F,FORMAT(cr, 2) CREDIT_F FROM entry WHERE DATE(`DATE`) >= STR_TO_DATE('$tgl_start', '%d-%m-%Y') AND DATE(`DATE`) <= STR_TO_DATE('$tgl_end', '%d-%m-%Y') AND lokasi = '$sess_lokasi' $dv_start $acc  ORDER BY `DATE`,dr DESC";
                return $this->mips_gl->query($sql);
        ...
        function get_data_entry_head($periode_terkini,$tgl_start,$tgl_end,$divisi_start,$divisi_end,$noacc_start,$noacc_end){
            ...
            if($periode_terkini == 1){ //ini artinya pakai periode, filter tanggal tidak berlaku
                ...
                $sql = "SELECT ref,SUM(dr) AS DBT_NF,FORMAT(SUM(dr), 2) DBT,FORMAT(SUM(cr), 2) KRD,SUM(cr) KRD_NF FROM entry WHERE SUBSTR(periodetxt,1,6) = '$sess_periode' AND lokasi = '$sess_lokasi' $dv_start $acc GROUP BY ref ORDER BY `DATE` ASC ";
                return $this->mips_gl->query($sql);

            }else{

                $sql = "SELECT ref,SUM(dr) AS DBT_NF,FORMAT(SUM(dr), 2) DBT,FORMAT(SUM(cr), 2) KRD,SUM(cr) KRD_NF FROM entry WHERE DATE(`DATE`) >= STR_TO_DATE('$tgl_start', '%d-%m-%Y') AND DATE(`DATE`) <= STR_TO_DATE('$tgl_end', '%d-%m-%Y') AND lokasi = '$sess_lokasi' $dv_start $acc GROUP BY ref ORDER BY `DATE` ASC ";
                return $this->mips_gl->query($sql);
        ...
        function get_data_acct_buku_besar($periode_terkini,$tgl_start,$tgl_end,$divisi_start,$divisi_end,$noacc_start,$noacc_end){
            ...
            if($periode_terkini == 1){ //ini artinya pakai periode, filter tanggal tidak berlaku
                ...
                $sql = "SELECT 	a.noac,
                        ...
                        WHERE SUBSTR(a.periodetxt,1,6) = '$sess_periode' AND a.lokasi = '$sess_lokasi' $dv_start $acc
                ...
                $sql = "SELECT 	a.noac,
                        ...
                        FROM entry AS a
                        INNER JOIN noac AS b ON a.noac = b.noac 
                        WHERE DATE(a.`DATE`) >= STR_TO_DATE('$tgl_start', '%d-%m-%Y') AND DATE(a.`DATE`) <= STR_TO_DATE('$tgl_end', '%d-%m-%Y') AND a.lokasi = '$sess_lokasi' $dv_start $acc
                        GROUP BY a.noac ORDER BY a.noac ASC";
        ...
        function get_data_entry_buku_besar($periode_terkini,$tgl_start,$tgl_end,$divisi_start,$divisi_end,$noacc_start,$noacc_end){
            ...
            if($periode_terkini == 1){ //ini artinya pakai periode, filter tanggal tidak berlaku
                ...
                $sql = "SELECT *,DATE_FORMAT(`DATE`, '%d-%m-%Y') TGL,cr CREDIT_F2,dr DEBET_F2,FORMAT(dr, 2) DEBET_F,FORMAT(cr, 2) CREDIT_F FROM entry WHERE SUBSTR(periodetxt,1,6) = '$sess_periode' AND lokasi = '$sess_lokasi' $dv_start $acc ORDER BY noac ASC";
                return $this->mips_gl->query($sql);

            }else{

                $sql = "SELECT *,DATE_FORMAT(`DATE`, '%d-%m-%Y') TGL,cr CREDIT_F2,dr DEBET_F2,FORMAT(dr, 2) DEBET_F,FORMAT(cr, 2) CREDIT_F FROM entry WHERE DATE(`DATE`) >= STR_TO_DATE('$tgl_start', '%d-%m-%Y') AND DATE(`DATE`) <= STR_TO_DATE('$tgl_end', '%d-%m-%Y') AND lokasi = '$sess_lokasi' $dv_start $acc  ORDER BY noac ASC"; /*ORDER BY `DATE`,dr DESC*/
                return $this->mips_gl->query($sql);
        ...
        function get_data_entry_head_buku_besar($periode_terkini,$tgl_start,$tgl_end,$divisi_start,$divisi_end,$noacc_start,$noacc_end){
            ...
            if($periode_terkini == 1){ //ini artinya pakai periode, filter tanggal tidak berlaku
                ...
                $sql = "SELECT ref,SUM(dr) AS DBT_NF,FORMAT(SUM(dr), 2) DBT,FORMAT(SUM(cr), 2) KRD,SUM(cr) KRD_NF FROM entry WHERE SUBSTR(periodetxt,1,6) = '$sess_periode' AND lokasi = '$sess_lokasi' $dv_start $acc GROUP BY noac ORDER BY noac ASC ";
                return $this->mips_gl->query($sql);

            }else{

                $sql = "SELECT ref,SUM(dr) AS DBT_NF,FORMAT(SUM(dr), 2) DBT,FORMAT(SUM(cr), 2) KRD,SUM(cr) KRD_NF FROM entry WHERE DATE(`DATE`) >= STR_TO_DATE('$tgl_start', '%d-%m-%Y') AND DATE(`DATE`) <= STR_TO_DATE('$tgl_end', '%d-%m-%Y') AND lokasi = '$sess_lokasi' $dv_start $acc GROUP BY noac ORDER BY noac ASC ";
                return $this->mips_gl->query($sql);
...
//                }
    ...
    function get_nama_acct($data){
        $sqls = "SELECT nama FROM noac WHERE noac = '$data[acct]'";   
    ...
    function get_coa_assets($thn,$bln){
        ...
        $sql = "SELECT  saldo01c,
                        ...
                        `group`,
                        `type`
                 FROM noac WHERE `group` = 'Asset' ORDER BY noac ASC";
        return $this->mips_gl->query($sql);
    ...
    function get_coa_capital($thn,$bln){
        ...
        $sql = "SELECT  saldo01c,
                        ...
                        `group`,
                        `type`
                 FROM noac WHERE `group` = 'Capital' ORDER BY noac ASC";
        return $this->mips_gl->query($sql);
    ...
    function get_coa_expenses($thn,$bln){
        ...
        $sql = "SELECT  saldo01c,
                        ...
                        `group`,
                        `type`
                 FROM noac WHERE `group` = 'Expenses' ORDER BY noac ASC";
        return $this->mips_gl->query($sql);
    ...
    function get_coa_liability($thn,$bln){
        ...
        $sql = "SELECT  saldo01c,
                        ...
                        `group`,
                        `type`
                 FROM noac WHERE `group` = 'Liability' ORDER BY noac ASC";
        return $this->mips_gl->query($sql);
    ...
    function get_coa_other_expenses($thn,$bln){
        ...
        $sql = "SELECT  saldo01c,
                        ...
                        `group`,
                        `type`
                 FROM noac WHERE `group` = 'Other Expenses' AND noac NOT IN ('950000000000000','950500000000000','950600000000000','950800000000000','951000000000000') ORDER BY noac ASC";
        return $this->mips_gl->query($sql);
    ...
    function get_coa_other_revenue($thn,$bln){
        ...
        $sql = "SELECT  saldo01c,
                        ...
                        `group`,
                        `type`
                 FROM noac WHERE `group` = 'Other Revenue' AND noac IN ('901000000000000','900900000000000','900100000000000','900450000000000','900500000000000','900700000000000') ORDER BY noac ASC";
        return $this->mips_gl->query($sql);
    ...
    function get_coa_revenue($thn,$bln){
        ...
        $sql = "SELECT  saldo01c,
                        ...
                        `group`,
                        `type`
                 FROM noac WHERE `group` = 'Revenue' AND noac IN ('600101010000000','600101050000000','600101100000000') ORDER BY noac ASC";
        return $this->mips_gl->query($sql);
