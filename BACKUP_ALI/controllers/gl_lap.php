...
class Gl_lap extends CI_Controller
{
    ...
    function trialbalance_by_detail($periode_terkini, $param2, $param3)
    {
        ...
        $data['data_list_assets']        = $this->gl_model->get_coa_assets($thn, $bln)->result_array(); //ok
        $data['data_list_capital']       = $this->gl_model->get_coa_capital($thn, $bln)->result_array(); //ok
        $data['data_list_expenses']      = $this->gl_model->get_coa_expenses($thn, $bln)->result_array(); //ok
        $data['data_list_liability']     = $this->gl_model->get_coa_liability($thn, $bln)->result_array(); //ok
        $data['data_list_other_expenses'] = $this->gl_model->get_coa_other_expenses($thn, $bln)->result_array(); //ok
        $data['data_list_other_revenue'] = $this->gl_model->get_coa_other_revenue($thn, $bln)->result_array(); //ok
        $data['data_list_revenue']       = $this->gl_model->get_coa_revenue($thn, $bln)->result_array(); //ok
